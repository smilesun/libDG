...
class BBBConv2d(_ConvNd):
    ...
    def convprobforward(self, input):
        ...
        # sample from output
        epsilon = torch.randn(conv_qw_mean.size())
        epsilon = epsilon.to(conv_qw_mean.device)
        output = conv_qw_mean + conv_qw_std * epsilon
...
class BBBLinearFactorial(nn.Module):
    ...
    def fcprobforward(self, input):
        ...
        # sample from output
        epsilon = torch.randn(fc_qw_mean.size())
        epsilon = epsilon.to(fc_qw_mean.device)

        output = fc_qw_mean + fc_qw_si * epsilon
